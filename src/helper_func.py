...
def tupleConstruct(operand, operator, weights_gates, settings):
    ...
    optr = seeded_rng().choice(operator, p=weights_gates)
    val = settings["vals"].get(optr)
    params = settings["par_nums"].get(optr)
    if params == 1:
        p = list(seeded_rng().random(1))
    else:
        p = list(seeded_rng().random(params))

    if val == 2:
        if params == 0:
            tpl = (optr, seeded_rng().choice(operand, 2, False))
        else:
            tpl = (optr, seeded_rng().choice(operand, 2, False), p)

    elif val == 1:
        if params == 0:
            tpl = (optr, [seeded_rng().choice(operand)])
        else:
            tpl = (optr, [seeded_rng().choice(operand)], p)
...
def quantum_state(settings, circ, ind, qubits=None, method=None, opt_within=None):
    ...
    if var == 1:
        ...
        if settings["use_numerical_optimizer"] == "yes":
            ...
            res = minimize(difference, init_parameters, args=(settings, circ), method=method)
            ...
            for child in offspring:
                if len(child) < min:
                    for n in range(min - len(child)):
                        gate = tupleConstruct(
                            settings["operand"], settings["operator"], settings["weights_gates"], settings
                        )
